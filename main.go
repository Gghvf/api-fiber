...
// isUserExists проверяет, есть ли логин в системе (в User или Admin)
...
func main() {
	...
	// ИСПРАВЛЕННЫЙ ЭНДПОИНТ /admin/create
	app.Get("/admin/create", func(c *fiber.Ctx) error {
		...
		if findAdmin(login) {
			return c.Status(409).JSON(fiber.Map{"error": "Пользователь уже является администратором"})
		}

		// Если пользователь НЕ существует в системе вообще, создаем для него базовую запись
		// Это позволяет создать админа "с нуля", если он еще не регистрировался
		if !isUserExists(login) {
			data.User = append(data.User, login)
		...
		// Добавляем в список админов (наделяем правами)
		...
		return c.JSON(fiber.Map{"message": "Администратор создан/обновлен", "login": login})
	})
