...
func main() {
	...
	app.Get("/admin/create", func(c *fiber.Ctx) error {
		...
		data.Kolvo = append(data.Kolvo, 1) // По умолчанию 1 персона
		mu.Unlock()
	...
	app.Get("/room/create", func(c *fiber.Ctx) error {
		...
		mu.Lock()
		// Проверка на дубликат номера комнаты
		...
		data.Status = append(data.Status, false) // ВАЖНО: Новая комната всегда свободна
	...
	app.Get("/rooms", func(c *fiber.Ctx) error {
		...
		for i := range data.Num {
			...
			rooms = append(rooms, fiber.Map{
				...
				"status":    statusText,
				"is_busy":   data.Status[i], // Добавляем сырой статус для отладки
				"date":      data.Date[i],
	...
	// --- ОТЛАДОЧНЫЙ ЭНДПОИНТ ---
	// Вызовите GET /debug/status?login=admin_login, чтобы увидеть raw данные
